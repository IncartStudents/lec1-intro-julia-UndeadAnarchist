...
const PI = 3.14159
PI = 3.14
#PI уже константа и, когда мы пытаемся изменить значение, язык локает
const PI = 3.14159
Pi = 3.14 #сработает, так как переменные разные, если уберем const, тоже сработает
...
b = 2.0
c = a + b
# Переменные становятся типа float, так как у нас целое и дробное значения, и происходит изменение типа переменной, чтобы значение после точки не потерялось, 
#для проверки можно добавить typeof, если b=2, тип будет Int64, если b=2.0, тип сменится на Float64



# Что теперь произошло с переменной а? Как происходит биндинг имен в Julia?
a = "foo"
#а теперь типа String, названия переменных привязаны к значениям, а не к типам, поэтому тип переменной можно всегда поменять
...
g::Int = 1
g = "hi"
#мы сказали, что g может быть только типа Int, а потом попытались загнать в него слово
#биндинг ограничен ::
...
greet()
#здесь мы создали локальную переменную, поэтому в общем коде ошибок не будет
...
v = "hello"
z
#Присваивание-переменная ссылается на объект, мутация-меняется сам объект
